...
function devEnv() {
  /*
   * .env has to be read explicitly. Astro loads it into import.meta.env for
   * the app, NOT into process.env, so the handler saw no CONTACT_RECIPIENT and
   * threw "not configured" on the first request. The empty prefix loads every
   * key, not just the PUBLIC_ ones - this is a dev-only node process and the
   * handler needs the unprefixed names.
   */
